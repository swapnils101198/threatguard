// extension/src/lib/threat-apis.ts
...
/**
 * Backend response shape. The backend is the only component that holds
 * API keys — the extension never sees them.
 */
 ...
 *
 * The backend fans out to Google Safe Browsing, VirusTotal, and PhishTank
 * in parallel and returns a normalized list of signals. It always responds
 * 200 with whatever sources succeeded; per-source failures are represented
 * as missing signals, not as HTTP errors.
 *
...
/**
 * Runtime type guard for the backend response.
 * Treats all network data as untrusted.
 */
